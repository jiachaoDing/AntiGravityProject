...
// --- Tokenizer Logic (Duplicated to avoid importing from src which might have other deps) ---
class TokenizerService {
    ...
    public tokenize(text: string): string {
        ...
        const result = this.segment.doSegment(cleanText, { simple: true });
        return result
            .filter((word: string) => {
                const w = word.trim();
...
const DEFAULT_DB_PATH = path.join(process.env.APPDATA || '', 'electron-ai-browser', 'history.db');
...
async function main() {
    ...
    try {
        const count = db.prepare('SELECT count(*) as count FROM messages_v2').get() as { count: number };
        console.log(`📊 Found ${count.count} messages to process.`);
    } catch (e) {
        console.error('❌ messages_v2 table not found. Is the database initialized?');
        process.exit(1);
    }

    // Prepare statements
    const getAllMessages = db.prepare('SELECT rowid, content FROM messages_v2');
    const insertFts = db.prepare('INSERT OR REPLACE INTO messages_fts_index (rowid, tokens) VALUES (?, ?)');

    const messages = getAllMessages.all() as { rowid: number, content: string }[];
    ...
    const runTransaction = db.transaction((msgs) => {
        for (const msg of msgs) {
            const tokens = tokenizer.tokenize(msg.content);
            insertFts.run(msg.rowid, tokens);
            processed++;
            if (processed % 100 === 0) {
                process.stdout.write(`\r⏳ Processed ${processed}/${total} messages...`);
            }
        }
    });
    ...
    try {
        ...
        console.log(`\n✅ Successfully re-indexed ${processed} messages!`);
    } catch (error) {
